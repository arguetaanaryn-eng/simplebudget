...
// v2.24.3: Strong live summary updates + repaint after deletes + sticky txn delete
(function(){
  function cur(){ const sel=document.getElementById('monthSelect'); return sel && sel.value && data.months ? data.months[sel.value] : null; }
  function getDebtWrap(){ return document.getElementById('debtCards'); }
  function raf(fn){ try{ return requestAnimationFrame(fn); }catch(_){ return setTimeout(fn, 0); } }
  function forceReflow(el){ if(!el) return; void el.offsetHeight; }

  // Debounced full render after any input in main sections (desktop + mobile)
  let _editTimer=null;
  ...
  ['input','change','blur','keyup'].forEach(ev=>{
    document.addEventListener(ev, (e)=>{
      ...
      if(el.closest('#incomeCards, #expenseCards, #debtCards')) scheduleFullRender();
    }, true);
  });

  // After any delete click, force a repaint + render (fix rows lingering until scroll)
  document.addEventListener('click', (e)=>{
    const del = e.target.closest && e.target.closest('.icon-btn.danger, .btn-delete, .delete, .trash, .txn-delete');
    if(!del) return;
    const host = document.querySelector('#incomeCards, #expenseCards, #debtCards');
    setTimeout(()=>{
      try{ if(window.renderAll) window.renderAll(); }catch(_){}
      raf(()=>forceReflow(host));
    }, 80);
  }, true);

  // Sticky per-row Delete for debt transactions (covers Safari scroll/virtualization)
  function addTxnDeleteToRow(row){
    if(!row || row.querySelector('.txn-delete')) return;
    const btn = document.createElement('button');
    btn.type = 'button'; btn.className = 'txn-delete'; btn.textContent = 'Delete';
    row.appendChild(btn);
  }
  function ensureTxnDeletes(){
    const wrap = getDebtWrap(); if(!wrap) return;
    const lists = wrap.querySelectorAll('.debt-transactions, #debtTransactions, [data-debt-txns]');
    lists.forEach(list=> list.querySelectorAll('.txn-row, .debt-txn-row, li, .row, [data-txn-row], tr').forEach(addTxnDeleteToRow));
  }
  function bindDelegatedDelete(){
    const wrap = getDebtWrap(); if(!wrap) return;
    if(wrap.__txnDelBound) return; wrap.__txnDelBound = true;
    wrap.addEventListener('click', function(e){
      const del = e.target.closest && e.target.closest('.txn-delete'); if(!del) return;
      const m = cur(); if(!m) return;
      const row = del.closest('.txn-row, .debt-txn-row, li, .row, [data-txn-row], tr'); if(!row) return;
      const list = row.parentElement;
      const rows = Array.from(list.querySelectorAll('.txn-row, .debt-txn-row, li, .row, [data-txn-row], tr'));
      const idx = rows.indexOf(row);
      const arr = m.debentx || m.debtTransactions || m.debtTxns || [];
      if(idx>=0 && arr[idx]){
        if(confirm('Delete this debt transaction?')){
          arr.splice(idx,1);
          if(m.debentx) m.debentx = arr;
          else if(m.debtTransactions) m.debtTransactions = arr;
          else if(m.debtTxns) m.debtTxns = arr;
          try{ saveData(); }catch(_){}
          try{ if(window.renderAll) window.renderAll(); }catch(_){}
        }
  ...
  document.addEventListener('change', (e)=>{
    if(e.target && e.target.id==='monthSelect'){ ensureTxnDeletes(); if(window.__renderSummary) window.__renderSummary(); }
  });
})();
